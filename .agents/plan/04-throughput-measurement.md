...
### Findings
- **[P1] Tail loss cannot be measured from `highest received seq + 1`.** If
  the final transmitted chunks are lost, the receiver never learns their
  sequence numbers and reports them as never sent, understating loss.
- **[P1] The terminal protocol is incomplete.** Cancel has no defined peer
  propagation, normal success has no transition to Phase 1's
  `run-ended: complete`, and `run-ended` can race result finalization and
  teardown.
- **[P2] Stage-message ownership contradicts itself.** The plan says slot 0
  emits every stage message and slot 1 never does, but both peers must emit
  `stage-armed` and `stage-complete`.
- **[P2] The pairing-time timestamp conflicts with the schema's completion
  timestamp definition.**

### Required Updates
1. Carry the sender's measured-chunk total through a reliable stage contract
   and define the incomplete-stage fallback when that total never arrives.
2. Specify one idempotent success/cancel/failure FSM covering peer
   propagation, partial save ordering, teardown, and the lifecycle-only DO
   completion signal.
3. State that only `stage-prepare` and `stage-start` are coordinator-only;
   both peers emit their own armed/complete acknowledgements.
4. Resolve the timestamp definition consistently with Phase 2 and the
...
### Assessment
- **P1 (loss denominator)**: Resolved. Every sender reports
  `sentMeasuredChunks` over reliable `stage-complete`; lost tail chunks are
  included, and a missing total makes the edge unavailable instead of
  inventing a denominator.
- **P1 (terminal protocol)**: Resolved. `test-abort`, the run-scoped
  idempotent finalizer, freeze/save/ack/teardown ordering, and Phase 1's
  payload-free `run-finished` cover success, cancel, failures, and races.
- **P2 (stage ownership)**: Resolved. Only prepare/start are
  coordinator-owned; armed/complete are emitted by both peers.
- **P2 (timestamp)**: Resolved. Phase 2, Phase 4, the main plan, and the
  authoritative schema now define the field as slot 0's canonical run
  timestamp.
...
### Findings
- **[P1] The promised directional pair is not available after a duplex
  failure.** Measurements are exchanged only in the terminal
  `result-share`; if one peer closes during duplex, the survivor never
  received that peer's directional measurement and can retain only one
  directional edge, contrary to this plan and Phase 5's matrix.
- **[P1] `saveResult` is not atomic across same-origin tabs.** The standard
  two-tab test can make both peer identities read and rewrite the same
  `localStorage` array concurrently, silently losing one otherwise valid
  record.
- **[P1] Both peers still cannot see receiver-observed live throughput and
  loss.** The receiver exposes throughput locally, but no bounded P2P
  progress message carries its measurements to the directional sender, as
  required by the main plan.
- **[P2] The binary header has no encoding for its end marker.** Its layout
  defines `phase` only as `0` or `1`, then later requires an unspecified
  terminal flag.

### Assessment
- All four findings from the previous review are resolved: the loss
  denominator, terminal FSM, stage ownership, and timestamp now align.

### Required Updates
1. Exchange and bank each completed directional measurement before duplex,
   or change every pair-retention claim to the survivor's actual one edge.
2. Serialize cross-tab writes or use independently keyed records, and test
   simultaneous saves from two tabs with different peer identities.
3. Add a bounded receiver-progress contract and UI path so both peers see
   live receiver-observed throughput and loss.
4. Define the end-marker bit/variant in the binary frame layout and its
   parser tests.
...
### Assessment
- **P1 (directional retention)**: Resolved. Every sealed receiver edge is
  replicated with `stage-result`, validated and banked by the other peer,
  and acknowledged before slot 0 may advance. Both directional edges are
  therefore present on both peers before duplex.
- **P1 (cross-tab persistence)**: Resolved. Results now use independently
  keyed IndexedDB rows and transactional `add`, with simultaneous
  different-identity and same-identity two-tab tests.
- **P1 (live metrics)**: Resolved. Receiver-owned
  `measurement-progress` is capped at four updates per second plus a final
  update and feeds the same fixed-edge UI on both browsers. Prefix loss is
  labelled provisional until the exact stage result is sealed.
- **P2 (end marker)**: Resolved. The 22-byte frame header defines
  `kind = 2` as a zero-payload end variant, with explicit invalid-frame
  parser tests.
