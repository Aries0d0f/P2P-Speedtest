...
/**
 * A component-local error boundary for the optional live visualization (6.5).
 *
 * Deliberately narrow: it replaces *only* the dashboard subtree, never
 * dispatches a room action, and is a sibling of the core test panel so the
 * metrics and Cancel button cannot be inside anything it can unmount.
 *
 * React error boundaries cannot see exceptions thrown later from an imperative
 * `requestAnimationFrame` or Anime.js callback. Those owners catch their own
 * and call `onVisualError`, which routes into the same fallback via `failed`.
 */
